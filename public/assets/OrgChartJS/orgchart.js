...
OrgChart.prototype._j = function (n, a, b, f) {
    var p = this;
    ...
    for (var j in this.response.visibleNodes) {
        var e = this.response.visibleNodes[j];
        h += this.ui.node(e, this._l(j), this.response.animations, this.config);
    ...
    var i = this.ui.svg(this.width(), this.height(), r, this.config, h);
    if (!this._I) {
        this.element.innerHTML = BALKANGraph._a(this.config.padding) + this.ui.css() + this.ui.exportMenuButton(this.config) + i;
        this._d();
        this._e();
        ...
        var l = o.parentNode;
        l.removeChild(o);
        l.insertAdjacentHTML("afterbegin", i);
        this._e();
        this.xScrollUI.addListener(this.getSvg());
        this.yScrollUI.addListener(this.getSvg());
        this.xScrollUI.setPosition();
...
OrgChart.ui = {
    ...
    node: function (c, e, b, d, q, s, k) {
        ...
        for (var i = 0; i < c.tags.length; i++) {
            if (BALKANGraph.icons.hasOwnProperty(c.tags[i])) {
                l += '<g transform="matrix(1,0,0,1,152,4)" control-node-menu-id="1">' + BALKANGraph.icons[c.tags[i]] + '</g>';
            }
        }
    ...
    expandCollapse: function (a, b) {
        ...
        if (a.state == BALKANGraph.COLLAPSE || a.tags.indexOf('more') >= 0) {
            c += BALKANGraph.expcollOpenTag.replace("{id}", a.id).replace("{x}", e).replace("{y}", f);
            c += d.plus;
            a.state = BALKANGraph.COLLAPSE;
...
OrgChart.server._O = function (j, k, a, e, b, g) {
    ...
    var d = function (o, i) {
        ...
        if (i == true && k[o].tags.indexOf('more') < 0) {
            if (k[o].children.length) {
                for (var p = 0; p < k[o].children.length; p++) {
...
OrgChart.prototype._m = function (d, b) {
    ...
    if (a.state == BALKANGraph.EXPAND) {
        this.collapse(c, this.config.deep)
    } else {
        if (a.state == BALKANGraph.COLLAPSE) {
            if (typeof this.config.plusMouseCLickBehiviour === 'function') {
                this.config.plusMouseCLickBehiviour(a, this);
            } else {
                this.expand(c, this.config.deep)
            }
        }
    }
};
...
BALKANGraph._aC = function () {
    for (var d = myText.childNodes.length - 1; d >= 0; d--) {
        myText.removeChild(myText.childNodes.item(d))
    }
    var f = 10;
    var m = myRect.getAttribute("width") - f;
    var o = +myRect.getAttribute("x");
    var p = +myRect.getAttribute("y");
    var a = +myText.getAttribute("font-size");
    var g = myTextValue.value;
    var n = g.split(" ");
    var h = document.getElementById("myText");
    var l = document.createElementNS(NS, "tspan");
    var j = document.createTextNode(n[0]);
    l.setAttribute("x", o + f);
    l.setAttribute("y", p + f + a);
    l.appendChild(j);
    h.appendChild(l);
    for (var c = 1; c < n.length; c++) {
        var e = l.firstChild.data.length;
        l.firstChild.data += " " + n[c];
        if (l.getComputedTextLength() > m - f) {
            l.firstChild.data = l.firstChild.data.slice(0, e);
            var l = document.createElementNS(NS, "tspan");
            l.setAttribute("x", o + f);
            l.setAttribute("dy", a);
            j = document.createTextNode(n[c]);
            l.appendChild(j);
            h.appendChild(l)
        }
    }
    var b = h.getBBox().height + 2 * f;
    myRect.setAttribute("height", b);
    showSourceSVG()
};
OrgChart.templates = {};
...
OrgChart.templates.rony.svg = '<svg  style="background-color:#e1e5ec;display:block;" width="{w}" height="{h}" viewBox="{viewBox}">{content}</svg>';
...
OrgChart.prototype._ax = function (d, b) {
    ...
    if (typeof this.config.nodeMouseClickBehaviour === 'function') {
        this.ripple(a.id, b[0].clientX, b[0].clientY)
        this.config.nodeMouseClickBehaviour(a, this);
    }
};
...
OrgChart.searchUI.createInputField = function (a) {
    return '<div data-id="search" style="display:none;border-radius: 20px 20px;padding:5px; box-shadow: #808080 0px 1px 2px 0px; font-family:Roboto-Regular, Helvetica;color:#7a7a7a;font-size:14px;border:1px solid #d7d7d7;width:300px;position:absolute;top:' + a + "px;left:" + a + 'px;background-color:#ffffff;"><div><div style="float:left;">' + BALKANGraph.icon.search(32, 32) + '</div><div data-id="cell-1" style="float:right; width:83%"><input placeholder="' + BALKANGraph.SERACH_TEXT + '" style="font-size:14px;font-family:Roboto-Regular, Helvetica;color:#7a7a7a;width:100%;border:none;outline:none; padding-top:10px;" type="text" /></div><div style="clear:both;"></div></div><div data-id="container"></div></div>'
};
